  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex items-center space-x-3 mb-6 pb-4 border-b border-slate-200 dark:border-slate-800">
        ...
        <div>
          <h1 className="text-lg font-bold text-slate-900 dark:text-white">Admin & AI Moderation</h1>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5 mb-8">
          <div className="p-3.5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
            ...
            <span className="text-xl font-extrabold text-slate-900 dark:text-white">{stats.totalListings}</span>
          </div>

          <div className="p-3.5 rounded-2xl bg-amber-50/60 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/60">
            <span className="text-[10px] uppercase font-bold text-amber-700 dark:text-amber-300 block">Flagged Items</span>
            <span className="text-xl font-extrabold text-amber-600 dark:text-amber-400">{stats.flaggedCount}</span>
          </div>

          <div className="p-3.5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
            <span className="text-[10px] uppercase font-bold text-slate-400 block">Campus Users</span>
            <span className="text-xl font-extrabold text-indigo-600 dark:text-indigo-400">{stats.userCount}</span>
          </div>

          <div className="p-3.5 rounded-2xl bg-indigo-50/60 dark:bg-indigo-950/30 border border-indigo-200 dark:border-indigo-800/60">
            <span className="text-[10px] uppercase font-bold text-indigo-700 dark:text-indigo-300 block">AI Filter Rate</span>
            <span className="text-xl font-extrabold text-emerald-600 dark:text-emerald-400">{stats.aiModerationRate}</span>
          </div>
        </div>
      )}
      ...
      {/* Flagged Section */}
      <div className="mb-8">
        <h2 className="text-sm font-bold text-slate-900 dark:text-white mb-3 flex items-center gap-1.5">
        ...
        {flaggedItems.length > 0 ? (
          <div className="space-y-3">
            {flaggedItems.map((item) => (
              <div key={item._id} className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-amber-200 dark:border-amber-800/60 flex flex-col md:flex-row md:items-center justify-between gap-3">
          ...
          <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-center text-xs text-slate-400">
            No flagged content currently in queue. AI safety filters active.
          </div>
        )}
      </div>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/80">
              {usersList.map((u) => (
                <tr key={u.id || u._id}>
                  ...
                  <td className="p-3 text-indigo-600 dark:text-indigo-400">{u.college}</td>
