...
export const ListingDetailModal = () => {
  const { selectedListing, closeListingDetail, wishlist, toggleWishlist } = useListingStore();
  const { isAuthenticated, openAuthModal } = useAuthStore();
  const { startOrOpenChat } = useChatStore();

  if (!selectedListing) return null;

  const isSaved = wishlist.includes(selectedListing._id);
            ...
            <div className="flex items-center space-x-2.5 pt-3 border-t border-slate-200 dark:border-slate-800">
              <button
                ...
                className={`p-2.5 rounded-xl border transition ${
                  isSaved 
                    ? 'bg-pink-50 dark:bg-pink-950/60 border-pink-200 dark:border-pink-800 text-pink-500' 
                    : 'bg-slate-100 dark:bg-slate-900 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-800 hover:text-pink-500'
                }`}
